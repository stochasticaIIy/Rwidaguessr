...
export async function onRequestPost({ request, env }) {
  ...
  if (!payload || !signature || !secureEqual(signature, await sign(payload, env.GAME_SIGNING_SECRET))) return Response.json({ error: 'Invalid game token.' }, { status: 403 });
  ...
  let listings; try { listings = JSON.parse(env.LISTINGS_JSON); } catch { return Response.json({ error: 'Invalid listings data.' }, { status: 500 }); }
