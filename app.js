(() => {
  ...
  const localized = (value) => {
    ...
    if (state.language === 'ar' && typeof res === 'string') {
      ...
      if (res === 'ديزل' || res.toLowerCase() === 'diesel') return 'مازوط';
      if (res === 'بنزين' || res.toLowerCase() === 'petrol' || res.toLowerCase() === 'essence') return 'ليصانص';
      if (res === 'هجين' || res.toLowerCase() === 'hybrid' || res.toLowerCase() === 'hybride') return 'إيبريد';
      if (res.toLowerCase() === 'electric' || res.toLowerCase() === 'electrique' || res.toLowerCase() === 'électrique') return 'كهربائي';
    }
    return res;
  };
